...
# Step through the list of all virtual machines in subscription and enforce the value to the tag.

foreach($vm in $resourceManagerVMList)

    {

        $taglist = @{AutoShutdownSchedule="ToBeConfigured"}

        $schedule = $null

 

        # Check if the AutoShutdownScheule tag has a null value.

        if($vm.ResourceType -eq "Microsoft.Compute/virtualMachines" -and $vm.Tags.AutoShutdownSchedule -eq "")

        {

            # Set the AutoShutdownSchedule value to ToBeConfigured.

            Set-AzResource -ResourceGroupName $vm.ResourceGroupName -Name $vm.name -ResourceType "Microsoft.Compute/VirtualMachines" -Tag $taglist -Force

            Write-Output "[$($vm.Name)]: Added the default value of ToBeConfigured to the AutoShutdownSchedule Tag."
